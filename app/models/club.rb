class Club < ApplicationRecord
  ...
  def count_result_on(year, game_result)
    year = year = Date.new(year, 1, 1)
    matches_on = matches.where(kicked_off_at: year.all_year)

    count = 0

    case game_result
    when "win"
      matches_on .each{|match| count += 1 if won?(match) }
    when "lost"
      matches_on .each{|match| count += 1 if lost?(match) }
    when "draw"
      matches_on .each{|match| count += 1 if draw?(match) }
    end
      count
  end
